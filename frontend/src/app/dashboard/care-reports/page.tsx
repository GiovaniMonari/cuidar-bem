...
import { ChevronLeft, Loader2, AlertCircle, FileText, Calendar, Clock, MapPin, Link, ChevronRight } from 'lucide-react';
...
export default function CareReportsPage() {
  ...
  const permissions = useMemo(() => {
    ...
    const clientUserId = 
      (booking?.clientId as any)?._id ||
      booking?.clientId;

    const isCaregiverOfBooking = isCaregiver && 
      String(caregiverUserId) === String(userIdToCompare);
    
    const isClientOfBooking = !isCaregiver && 
  ...
  // Calcular informações do serviço
  const serviceInfo = useMemo(() => {
    ...
    const isServiceActive = ['in_progress', 'confirmed'].includes(booking.status);
    const isServiceDone = booking.status === 'completed';

    const canSendReport = permissions.isCaregiverOfBooking && (
    ...
    const nextDayToReport = isMultiDay 
      ? Array.from({ length: currentDay }, (_, i) => i + 1)
          .find(d => !daysWithReport.includes(d)) || currentDay
      : 1;
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-yellow-900 text-sm">Acesso não autorizado</h3>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-6">
            ...
            {!serviceInfo?.canSendReport && (
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
                ...
                <p className="text-sm text-gray-600">
                  {booking.status === 'pending' && 'Aguardando confirmação do serviço.'}
                  {booking.status === 'confirmed' && !serviceInfo?.isMultiDay && 'O relatório poderá ser enviado após a conclusão do serviço.'}
                  {booking.status === 'cancelled' && 'Este serviço foi cancelado.'}
                </p>
              </div>
            )}
          </div>
        ...
        {permissions.isClientOfBooking && (
          <div className="space-y-4">
            {/* Aviso quando não há relatórios */}
            {reports.length === 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <Calendar className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-blue-900 font-medium text-sm">Aguardando relatórios</p>
                    <p className="text-blue-700 text-xs mt-1">
                      O cuidador enviará relatórios sobre os cuidados prestados. Você será notificado quando um novo relatório for enviado.
                    </p>
                  </div>
                </div>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${bg} ${text} flex-shrink-0`}>
      {label}
    </span>
  );
}
...
export default function CareReportsHistoryContent() {
        ...
        {filteredReports.length === 0 ? (
          ...
          <div className="space-y-4">
            {Object.entries(groupedReports || {}).map(
              ([bookingId, { booking, reports: bookingReports }]) => (
                <div
                  key={bookingId}
...
function ReportCard({
  ...
  const MoodIcon = report.patientMood
    ? report.patientMood <= 2
      ? Frown
      : report.patientMood === 3
        ? Meh
        : Smile
    : null;
        ...
        {showBookingInfo && report.bookingId && (
          <div className="text-xs text-gray-500 pb-2 border-b border-gray-100">
            <span className="font-medium text-gray-700">
              {(report.bookingId as any)?.serviceName}
            </span>

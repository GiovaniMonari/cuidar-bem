...
import { reportUserSchema, type ReportUserFormData } from '@/validations/schemas';
...
export function ReportUserModal({
  ...
  const {
    ...
    resolver: yupResolver(reportUserSchema) as any,
    mode: 'onBlur',
    defaultValues: {
  ...
  const onSubmit = async (data: ReportUserFormData) => {
    ...
    try {
      await api.createPlatformReport({
        ...
        bookingId,
        conversationId,
        reason: data.reason as PlatformReportReason,
            ...
            <div className="space-y-3">
              {REASONS.map((item) => (
                <button
                  key={item.value}
                  type="button"
                  onClick={() => setValue('reason', item.value, { shouldValidate: true })}
